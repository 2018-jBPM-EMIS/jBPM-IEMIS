...
public class SScoreDao extends BaseDao {
	...
	public void addSScoreBatch(){
		String sql = "insert into sscore(user_id,course_id,score_year,score_year_half) "
				   + " SELECT user_id,course_id,scource_year,scource_year_half from scource "
				   + "  where (user_id,course_id,scource_year,scource_year_half) not in (SELECT user_id,course_id,score_year,score_year_half from sscore)";
		Object[] params = null;
	...
	public List<SScore>  listSScoresData1(SScore score){
		...
		StringBuilder sBuilder = new StringBuilder();
		sBuilder.append("SELECT IFNULL(score_value1,0) score_value1,IFNULL(score_value2,0) score_value2,IFNULL(score_value3,0) score_value3,IFNULL(score_value4,0) score_value4,IFNULL(score_value5,0) score_value5 FROM sscore s ");
