...
public class ScourceDao extends BaseDao {
	...
	public List<Scource>  listScources(Scource scource){
		...
		if (!StringUtil.isEmptyString(scource.getCourse_name())) {
			sBuilder.append(" and p.course_id in (select course_id from Course where course_name like '%" + scource.getCourse_name() +"%') ");
		}
		if (scource.getUser_id()!=0) {
			sBuilder.append(" and p.user_id = " + scource.getUser_id() +" ");
		}
		if (!StringUtil.isEmptyString(scource.getUser_name())) {
			sBuilder.append(" and p.user_id in (select user_id from User where user_name like '%" + scource.getUser_name() +"%') ");
		}
		if (!StringUtil.isEmptyString(scource.getReal_name())) {
			sBuilder.append(" and p.user_id in (select user_id from User where real_name like '%" + scource.getReal_name() +"%') ");
		}
